...
import { RoiChip } from './charts.jsx';
        ...
        <div className="cjwrap">
          <table className="cjtbl">
            <thead>
              <tr>
                <th>Conjunto</th>
                {cols.map((col) => <th key={col.k} className="r">{col.lb}</th>)}
              </tr>
            </thead>
            <tbody>
              {adsets.map((a) => (
                <tr key={a.adset_id}>
                  ...
                  {cols.map((col) => {
                    ...
                    if (col.k === 'roi') return <td className="r" key={col.k}>{v != null ? <RoiChip roi={v} /> : <span className="cj-na">—</span>}</td>;
